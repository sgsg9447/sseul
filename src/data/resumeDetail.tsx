        ...
        blocks: [
          {
            label: '배경',
            items: [
              'AI가 생성한 슬라이드를 사용자가 직접 편집·저장하는 신규 서비스로, 생성–편집–저장이 연결되는 일관된 흐름 설계 필요',
              <>
                편집형 서비스 특성상 작은 동작 차이도 전체 경험에 큰 영향을 주어, 기획서 이면의 예외 상황 정의가 품질의 핵심
                <br />
                예: 다중 선택 중 포맷 변경 · 저장 시점 충돌 · 빈 입력 처리처럼 기획서에 없던 동작
              </>,
            ],
          },
          {
            ...
            items: [
              ...
              <>
                편집 시스템을 특정 사업 형태에 종속되지 않는 모듈형 구조로 정의
                <br />→ B2C뿐 아니라 B2B 환경에서도 동일 자산을 재활용할 수 있도록 구조화
              </>,
              <>
                <L>구현 가능성 검증</L> — 핵심 구간을 PoC로 먼저 만들어 빠르게 검증하며 React·TypeScript로 개발을 이어감
              </>,
            ],
          },
          {
        ...
        blocks: [
          {
            ...
            items: [
              ...
              <>
                시험 시즌에는 <M>약 10만 건</M>을 단기간에 반영해야 했는데, 기존 방식으로는 공급 속도가 이를 따라가지 못함
              </>,
            ],
          },
          {
            ...
            items: [
              <>
                화면을 각각 만드는 대신 <M>웹에서 한 번 만든 결과를 이미지로 공통 사용하는 구조로 전환</M>
                <br />→ 앱 릴리즈 의존을 끊는 것을 목표로 설정
              </>,
        ...
        blocks: [
          {
            ...
            items: [
              <>
                실제 운영 사이트와 운영자를 대상으로 한 분석에서 출발.
                <br />
                콘텐츠가 데이터가 아닌 이미지·게시글로만 존재하고, 탐색→판단→신청 동선과 정보 위계가 설계되지 않은 상태
              </>,
              <>
          ...
          },
          {
            ...
            items: [
              <>
                과정을 <L>데이터로 모델링</L>하고 지원 유형(경기도 무료·국비·자부담)을 속성으로 담아,
                <br />
                수강생은 과정명만으로 신청하면 절차가 자동으로 갈라지도록 정보구조 설계
              </>,
              <>
                탐색→판단→신청으로 이어지는 전환 흐름과 신청 후 피드백(1영업일 내 전화)을 설계해,
                <br />
                확신이 형성되는 과정 상세 직후에 신청을 배치
              </>,
          ...
          },
          {
            ...
            items: [
              <>
                보여주기용 사이트를 <M>신청·운영까지 연결하는 업무 도구</M>로 재정의
              </>,
        ...
        blocks: [
          {
            ...
            items: [
              <>
                1순위 사용자 니즈는 "진짜 배워서 취업되나"였지만,
                <br />
                기관은 취업률을 집계하지 않아 가장 강력한 증거를 화면에 제시할 수 없는 상황
              </>,
            ],
          },
          {
            ...
            items: [
              <>
                취업을 "보장"하지 않고, 사용자가 스스로 "할 수 있겠다"고 판단할 <L>근거를 제공</L>하는 방향으로 결정
                <br />
                없는 수치를 만들지 않고 보유 자산(회차별 커리큘럼·자격증 경로·현장 증거·기관 인증)으로 확신을 구성
              </>,
