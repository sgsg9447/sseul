...
export const careerSummary: ReactNode = (
  <>
    프론트엔드 개발 경험을 바탕으로, 구현 가능성까지 고려해 제품을 설계하는 기획자입니다.
    <br />
    AI 편집 서비스 구조 설계, 콘텐츠 운영 백오피스 기획, 데이터 기반 UX 검증을 수행하며 모호한 요구사항을 구조로 정리하고
    의사결정으로 연결하는 역량을 쌓았습니다.
  </>
);
...
export const careerSkills: ReactNode = (
  <>
    <b>제품 설계</b> · 제품 구조 설계 · 사용자 흐름 설계 · 도메인 모델링
    <br />
    <b>데이터·검증</b> · 데이터 기반 의사결정(A/B·OKR) · 구현 가능성 검증(React·TypeScript)
    <br />
    <b>운영·협업</b> · 프로세스 자동화 기획 · 이해관계자 조율 · 문서화
  </>
);
...
export const careerCompanies: CareerCompany[] = [
    ...
    projects: [
      {
        ...
        rows: [
          ...
          },
          {
            ...
            value: (
              <>
                생성–편집–저장이 분절되면 편집 경험이 무너지고, 작은 동작 차이도 전체 경험에 영향.
                <br />
                동시에 서비스 방향(B2C/B2B)이 불확실한 상황
              </>
            ),
          },
          {
            ...
            value: (
              <>
                ...
                도출해 동작 기준을 먼저 확정.
                <br />
                사업 방향 불확실성에 대비해 시스템을{' '}
                <Em>특정 형태에 종속되지 않는 모듈형으로 설계<br />→ B2C·B2B 양쪽 재활용 가능</Em>하도록 판단.
                <br />
          ...
          },
          {
            ...
            value: (
              <>
                편집–미리보기–반영을 하나의 흐름으로 연결해 일관성·안정성 확보.
                <br />
                <Em>B2C 정식 출시</Em>, 동일 구조를 B2B로 확장 가능한 자산으로 정리
              </>
            ),
          },
        ],
      },
      {
        ...
        rows: [
          ...
          },
          {
            ...
            value: (
              <>
                아이콘 등 에셋을 수동 관리하며 추가 비용 증가·확장 한계,
                <br />
                협업 기준이 분산되어 커뮤니케이션 비용 발생
              </>
            ),
          },
          {
          ...
          },
          {
            ...
            value: (
              <>
                에셋 추가·변경 비용 감소(대상 아이콘 <Em>200개</Em>), UI 일관성 확보,
                <br />
                테마 변경 시 개별 수정 없이 전역 대응 가능
              </>
            ),
          },
        ],
    ...
    projects: [
      {
        ...
        rows: [
          { label: '담당 역할', value: '① 운영 병목 분석 ② 문제 도메인 모델링 ③ 에디션 운영 기준 수립' },
          {
            ...
            value: (
              <>
                매월 반복되는 제작·편집·배포를 수작업으로 처리해 생산성 저하,
                <br />
                과목/유형별 분류 요구 증가로 기존 방식 확장 한계
              </>
            ),
          },
          {
            ...
            value: (
              <>
                도구를 먼저 만들기보다 <Em>문제 데이터를 대/중/소단원 트리로 모델링해 운영 구조를 정의</Em>하고,
                <br />
                현업 실무자가 직관적으로 구성·재배치하도록 흐름 설계.
                <br />
                기존 문제집을 복사·버전화하는 ‘에디션’ 기준을 세워 재활용 가능하도록 판단
              </>
            ),
          },
          {
            ...
            value: (
              <>
                제작–편집–배포를 하나의 도구로 통합해 월 운영 효율 향상,
                <br />
                일회성 제작에서 벗어나 콘텐츠 자산화 구조 마련
              </>
            ),
          },
        ],
      },
      {
        ...
        rows: [
          ...
          },
          {
            ...
            value: (
              <>
                화면을 각각 만드는 대신 <Em>웹에서 한 번 만든 결과를 이미지로<br />공통 사용하는 구조로 전환</Em>해 앱 릴리즈 의존을 끊는 것을 목표로 잡고,
                <br />
                대량 적재의 성능·용량 문제를 미리 보고 시즌 중심 운영·삭제 정책을 함께 설계
              </>
            ),
          },
          {
            ...
            value: (
              <>
                콘텐츠 변경에 앱 출시가 불필요해져 출시 주기 단축·개발 리소스 절감,
                <br />
                시즌 대량 콘텐츠를 앱 출시 없이 적시 공급하는 운영 구조 확보
              </>
            ),
          },
        ],
    ...
    projects: [
      {
        ...
        rows: [
          ...
          },
          {
            label: '문제·배경',
            value:
              '팀이 설정한 OKR(검색 결과에 연관검색어 제공)에 따라, 추가 탐색을 유도해 거래 효율 향상 기능 도입',
          },
          {
            label: '수행·기여',
            decision: true,
  ...
  {
    name: 'KOSSA',
    meta: '국방부 SW·AI 교육 운영 · Product Manager · 2020.09 ~ 2021.10',
    projects: [
      {
        n: '01',
        title: '해커톤 운영을 위한 웹 기반 운영 기능 기획',
        keys: '멘토링 매칭 · 팀 그룹핑 · 운영 자동화',
        rows: [
          {
            label: '담당 역할',
            value: '집체교육·해커톤 운영 / 멘토링 매칭·팀 그룹핑을 홈페이지에서 처리하도록 서비스 기획',
          },
          {
            label: '문제·배경',
            value: '참가자 모집·팀 구성·멘토 매칭을 수기로 처리해 운영 부담·실수·지연 발생',
          },
          {
            label: '접근·의사결정',
            decision: true,
            value: '운영자가 반복하던 매칭·그룹핑을 웹에서 직접 처리하도록 기능·흐름 정의',
          },
          {
            label: '성과',
            value: '오프라인·수기 운영을 웹 흐름으로 전환해 운영 부담 절감 (정량 지표 미집계)',
          },
        ],
      },
    ],
